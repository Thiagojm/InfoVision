...
def main():
    ...
    window = sg.Window("InfoVision 1.1.2 - by Thiago Jung", layout, return_keyboard_events=True, use_default_focus=False,
    ...
    while True:
        ...
        if event == "START/STOP":
            if not ongoing:
                try:
                    ...
                    time.sleep(3)
                    act_image = pc_choose(values, image_dict)
                    if values["sound_on"]:
        ...
        if event in events_list:
            try:
                ...
                window['START/STOP'].update("START")
                ongoing = False
        if event == "k": # Control PC chooses the image and answer
            try:
                if ongoing:
                    pc_choosed = pc_choose(values, image_dict)
                    text_elem.update(pc_choosed)
                    if act_image == pc_choosed:
                        hit = 1
                        right_hits += 1
                    else:
                        hit = 0
                    percent = round(float(right_hits * 100 / count), 2)
                    with open(f'{doc_name}.csv', 'a+') as fd:
                        fd.write(f"{count}, {act_image}, {pc_choosed}, {hit}, {percent}\n")
                    percent = round(float(right_hits * 100 / count), 2)
                    percent_element.update(f"{right_hits} ({percent}%)")
                    count += 1
                    count_element.update(count)
...
def pc_choose(values, image_dict):
    if values["pseudo"]:
        return secrets.choice(list(image_dict.keys()))
    else:
        return trng3_random()
